...
def load_vgg(sess, vgg_path):
    ...
    t_keep_prob = graph.get_tensor_by_name(vgg_keep_prob_tensor_name)
    t_layer3_out = graph.get_tensor_by_name(vgg_layer3_out_tensor_name)
    t_layer4_out = graph.get_tensor_by_name(vgg_layer4_out_tensor_name)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        # placeholder for learning rate, input_image and correct_label
        t_learning_rate = tf.placeholder(tf.float32,
                                         name='learning_rate')
        t_correct_label = tf.placeholder(tf.float32,
